...
<div class="callout">
  <p><strong>Working behind a firewall?</strong> You'll need outbound HTTPS to <code class="inline">hub.docker.com</code> (Docker images) and <code class="inline">orchestack.ng</code> (installer + compose file). Everything else is internal.</p>
</div>
...
<ul>
  ...
  <li><strong>orchestack.ng</strong> — marketing + docs + the installer script you're about to run. The installer is a thin shell script that pulls from the other two places.</li>
</ul>
...
<pre>curl -fsSL https://orchestack.ng/install.sh | bash</pre>
...
<div class="callout">
  <p><strong>Pin to a specific version</strong> by passing <code class="inline">ORCHESTACK_VERSION=v0.1.1 curl -fsSL https://orchestack.ng/install.sh | bash</code>. Default is the latest stable tag.</p>
</div>
...
<p style="margin: 32px 0 14px"><strong>What Path D adds beyond Path A</strong>:</p>
<table class="docs-table">
  <colgroup>
    <col style="width: 22%">
    <col style="width: 28%">
...
def render_header(current: str) -> str:
    ...
    # Note: {up} is still used by the brand link. Services/Contact use absolute
    # https://orchestack.ng/ URLs so they resolve correctly when docs are served
    # from the docs.orchestack.ng subdomain (Cloudflare Pages rewrite maps
    # docs.orchestack.ng/* → /docs/*, which would otherwise break relative
    # ../services.html links from inside the docs tree).
    ...
    <nav class="nav">
      ...
      <div class="cta">
        <span class="nav-social">
          <a class="nav-icon" href="https://github.com/tripleaceme/orchestack-public"
             aria-label="GitHub" title="GitHub" target="_blank" rel="noopener">
